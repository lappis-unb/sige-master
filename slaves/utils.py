...
from measurements.models import MinutelyMeasurement, MonthlyMeasurement, QuarterlyMeasurement, RealTimeMeasurement, Tax
from transductors.models import EnergyTransductor

from .api import request_all_events, request_measurements
...
class CheckTransductorsAndSlaves:
    def check_transductors(self):
        ...
        for slave in slaves:
            ...
            url = f"http://{slave.ip_address}:" + os.getenv("SLAVE_PORT") + "/broken-transductors"
            try:
                ...
                if web_request.status == 200:
                    ...
                    for transductor in transductors:
                        transductor_status = EnergyTransductor.objects.get(id=transductor["id"])
...
class DataCollector:
    ...
    def build_monthly_measurements(msm, transductor):
        MonthlyMeasurement.objects.create(
            ...
            active_max_power_list_peak=(msm["active_max_power_list_peak"]),
            active_max_power_list_off_peak_time=(msm["active_max_power_list_off_peak_time"]),
            active_max_power_list_off_peak=(msm["active_max_power_list_off_peak"]),
            reactive_max_power_list_peak_time=(msm["reactive_max_power_list_peak_time"]),
            reactive_max_power_list_peak=(msm["reactive_max_power_list_peak"]),
            reactive_max_power_list_off_peak_time=(msm["reactive_max_power_list_off_peak_time"]),
            reactive_max_power_list_off_peak=(msm["reactive_max_power_list_off_peak"]),
    ...
    def build_events(events):
        for event_dict in events:
            ...
            transductor = EnergyTransductor.objects.get(ip_address=event_dict["ip_address"])
    ...
    def build_realtime_measurements(msm, transductor):
        if RealTimeMeasurement.objects.filter(transductor=transductor):
            measurement = RealTimeMeasurement.objects.get(transductor=transductor)
            ...
            measurement.collection_date = datetime.strptime(msm["transductor_collection_date"], "%Y-%m-%dT%H:%M:%S.%f")
            ...
            RealTimeMeasurement.objects.create(
                ...
                collection_date=datetime.strptime(msm["transductor_collection_date"], "%Y-%m-%dT%H:%M:%S.%f"),
            )
    ...
    def get_measurements(*args, **kwargs):
        ...
        for slave in slaves:
            if kwargs.get("realtime", None):
                realtime_response = request_measurements("realtime-measurements", slave)

                measurement = json.loads(realtime_response.content)
                for transductor_data in measurement:
                    try:
                        transductor = EnergyTransductor.objects.get(id=transductor_data["transductor_id"])
                        DataCollector.build_realtime_measurements(transductor_data, transductor)
            ...
            for transductor in slave.transductors.all():
                ...
                if kwargs.get("minutely", None):
                    ...
                    for msm in measurements:
                        ...
                        try:
                            DataCollector.build_minutely_measurements(msm, transductor)
                        except Exception as exception:
                            pass
                ...
                if kwargs.get("quarterly", None):
                    ...
                    for msm in measurements:
                        ...
                        # try:
                        DataCollector.build_quarterly_measurements(msm, transductor)
                        # except Exception:
                        # pass
                ...
                if kwargs.get("monthly", None):
                    ...
                    for msm in measurements:
                        ...
                        # try:
                        DataCollector.build_monthly_measurements(msm, transductor)
                        # except Exception:
                        #     pass
