...
def get_measurements_with_missing_values(
        measurements, value_field: str,
    ...
    if measurements and len(measurements):
        ...
        start_date_in_minutes = get_minutes_from_date(
            measurements[0]['collection_date']
        )
        end_date_in_minutes = get_minutes_from_date(
...
def get_missing_values_filled(
    missing_start=0, missing_end=0,
    missing_values_fill=0, missing_tolerance=10
) -> list:
    missing_values = []
    if abs(missing_start - missing_end) >= missing_tolerance:
        for missing_minute in range(missing_start, missing_end):
            missing_values.append([
                get_date_from_minutes(missing_minute),
                missing_values_fill
            ])
